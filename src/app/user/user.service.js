"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var http_1 = require('@angular/http');
var ng2_cookies_1 = require('ng2-cookies/ng2-cookies');
require('rxjs/add/operator/toPromise');
var util = require('../util/util');
var UserService = (function () {
    function UserService(http) {
        this.http = http;
        this.jsonHeaders = new http_1.Headers({
            'Content-Type': 'application/json'
        });
        this._user = null;
        this._returnCachedUser = false;
        this.userUpdated = new core_1.EventEmitter();
    }
    UserService.prototype.isUserLoggedIn = function () {
        return !!ng2_cookies_1.Cookie.get('loggedIn');
    };
    UserService.prototype.logout = function () {
        this.sendPutRequest(util.baseUrl + '/putLogout', {});
    };
    UserService.prototype.getUsers = function () {
        return this.http.get(util.baseUrl + '/getUsers')
            .toPromise()
            .then(function (response) { return response.json(); })
            .catch(this.handleError);
    };
    UserService.prototype.getActiveUsers = function () {
        return this.http.get(util.baseUrl + '/getActiveUsers')
            .toPromise()
            .then(function (response) { return response.json(); })
            .catch(this.handleError);
    };
    UserService.prototype.getUser = function () {
        var _this = this;
        if (this._returnCachedUser && this._user) {
            return new Promise(function (resolve, reject) {
                resolve(_this._user);
            });
        }
        else {
            var promise = this.http.get(util.baseUrl + '/getUser')
                .toPromise()
                .then(function (response) { return response.json(); })
                .catch(this.handleError);
            this._returnCachedUser = true;
            setTimeout(function () { _this._returnCachedUser = false; }, 120 * 60000);
            promise.then(function (user) { return _this._user = user; });
            return promise;
        }
    };
    UserService.prototype.putSaveTradeUrl = function (tradeUrl) {
        return this.sendPutRequest(util.baseUrl + '/saveTradeUrl', { tradeUrl: tradeUrl });
    };
    UserService.prototype.putTransferAmount = function (amount, toActive) {
        var url = toActive ? '/transferToActive' : '/transferToStatic';
        return this.sendPutRequest(util.baseUrl + url, { amount: amount });
    };
    UserService.prototype.sendPutRequest = function (url, data) {
        return this.http.put(url, JSON.stringify(data), { headers: this.jsonHeaders })
            .toPromise()
            .then(function (response) { return response.json(); })
            .catch(this.handleError);
    };
    UserService.prototype.updateUser = function (user) {
        this.userUpdated.emit(user);
        this._user = user;
    };
    UserService.prototype.handleError = function (error) {
        console.error('An error occurred', error);
        return Promise.reject(error.message || error);
    };
    UserService = __decorate([
        core_1.Injectable(), 
        __metadata('design:paramtypes', [http_1.Http])
    ], UserService);
    return UserService;
}());
exports.UserService = UserService;
//# sourceMappingURL=user.service.js.map